...
describe('Edition001 V2 Ultimate Test Cases', async () => {

  let Genius, StabilityPool, Edition, EditionV2, Royalty, Calendar, Penalty, Miners, Genft, AuctionHouse, LGenius;
  ...
  before(async () => {
    ...
    let tx = Edition.connect(alice).userMintUltimate(Genius.address, 2);
    await tx;

    await Genft.newEdition(Edition.address);

    describe('Ultimate Tests', async () => {
      ...
      it('defineUserMints has not been set', async () => {
        const tx = EditionV2.connect(alice).userMintUltimate(ethers.constants.AddressZero, 1);
        ...
        console.log("MintSetting: ", await Edition.listMintCollaterals(0));
      ...
      it('alice can mint Ultimate NFT with Native', async () => {
        let tx = EditionV2.connect(alice).userMintUltimate(ethers.constants.AddressZero, 1);
        ...
        await EditionV2.connect(alice).userMintUltimate(ethers.constants.AddressZero, 1,
        ...
        expect(contractNativeBalance).to.be.equal(ethers.utils.parseEther("0.1"));
      });
      ...
      it('Alice can mint Ultimate NFT with Genius Token', async () => {
        ...
        let tx = EditionV2.connect(alice).userMintUltimate(Genius.address, 1);
        await tx;
      ...
      it('Alice can mint Ultimate NFT with ERC20 Token (DAI)', async () => {
        // transfer Genius token to the user
        ...
        let tx = EditionV2.connect(alice).userMintUltimate(Dai.address, 1);
        await tx;
      ...
      it('Alice can convert V1 to V2', async () => {
        ...
        expect(await Edition.balanceOf(alice.address, ULTIMATE_TOKEN_ID)).to.be.equal(2);

        await Edition.connect(alice).setApprovalForAll(EditionV2.address, true);
        await EditionV2.connect(alice).convert();

        expect(await Edition.balanceOf(alice.address, ULTIMATE_TOKEN_ID)).to.be.equal(0);
        // 6 = existing v2 tokens: 3(using native, geni, dai) + converted v2 tokens: 2 + bonus for conversion: 1
        expect(await EditionV2.balanceOf(alice.address, ULTIMATE_TOKEN_ID)).to.be.equal(5);
        expect(await EditionV2.balanceOf(alice.address, BOOSTER_TOKEN_ID)).to.be.equal(1);
      });

      it('Royalty Balances Check', async () => {
        // Native Token
        //// 1 token using Native Token
        expect(await Royalty.balanceOf(ethers.constants.AddressZero)).to.be.equals(ethers.utils.parseEther("0.1"));
        //// 3 tokens sent to Genius OA beneficiary, 2 tokens from Edition001, 1 tokens from Edition001V2 for 2000,000 GENI
        expect(await Genius.balanceOf(await Genius.oaBeneficiary())).to.be.equals(2 * parse(1_000_000, 9));
        //// 1 token using ERC20 Dai,
        expect(await Dai.balanceOf(Royalty.address)).to.be.equals(parse(100, 18));
        expect(await Royalty.balanceOf(Dai.address)).to.be.equals(ethers.utils.parseEther("100"));
      });
  ...
  describe('canPurchase Modifier', () => {
    ...
    it('should throw ErrorPobOnly if less than 60 days from edition launch and paymentToken is anything other than Geni', async () => {
      ...
      */
      ...
      await expect(EditionV2.connect(alice).userMintUltimate(Dai.address, 1)).to.be.revertedWithCustomError(EditionV2,'ErrorPobOnly');
    });
    ...
    it('should throw ErrorEditionExpired if editionV2 is not latest and Genius Day is greater than 365', async () => {
      ...
      */
      ...
      await expect(EditionV2.connect(alice).userMintUltimate(Genius.address, 1)).to.be.revertedWithCustomError(EditionV2,'ErrorEditionExpired');
    });
